...
class Rotate360 extends Component {
  ...
  preloadImages = () => {
    ...
    for (let i = 1; i <= 20; i++) {
      let image = new Image();
      image.src = `https://polyreactsa.blob.core.windows.net/images/${currentRegion.toLowerCase()}/${currentRegion.toLowerCase()}-${activeExhibition}-${i}.jpg${sas}`;
      image.alt = i;
  ...
  renderImage = () => {
    ...
    let src =
      imgArr.length > 0
        ...
        : `https://polyreactsa.blob.core.windows.net/images/${currentRegion.toLowerCase()}/${currentRegion.toLowerCase()}-${activeExhibition}-1.jpg${sas}`;
